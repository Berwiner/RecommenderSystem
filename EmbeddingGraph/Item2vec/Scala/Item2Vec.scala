...
//      appName("Accuracy").
//      getOrCreate()
//    var data = spark.read.parquet(p.input_pt).drop("tid")
//    var colNames = data.columns
//    for (colName <- colNames) {
//      data = data.withColumn(colName, data.col(colName).cast(DoubleType))
//    }
//    val class_0_predict = spark.sparkContext.doubleAccumulator
//    val class_1_predict = spark.sparkContext.doubleAccumulator
//    val class_0_sum = spark.sparkContext.doubleAccumulator
//    val class_1_sum = spark.sparkContext.doubleAccumulator
//    import spark.implicits._
//    data.rdd.map {
//      case Row(predict: Double, label: Double) =>
//        if (predict < p.threshold) {
//          class_0_sum.add(1)
//        }
//        else {
//          class_1_sum.add(1)
//        }
//        if (label == 1 && predict > p.threshold) {
//          class_1_predict.add(1)
//
//        }
//        if (label == 0 && predict < p.threshold) {
//          class_0_predict.add(1)
//
//        }
//    }.count()
//
//    if(class_0_sum.value ==0 )
//      class_0_sum.add(1)
//    if(class_1_sum.value ==0)
//      class_1_sum.add(1)
//
//    val class_1_accuracy = class_1_predict.value / class_1_sum.value
//    val class_0_accuracy = class_0_predict.value / class_0_sum.value
//    val average_accuracy = (class_0_accuracy + class_1_accuracy) / 2
//    val sum_accuracy =  (class_0_predict.value+class_1_predict.value)/(class_0_sum.value+class_1_sum.value)
//
//    println("Class 0 Accuracy:" + class_0_predict + "/" + class_0_sum + " " + class_0_accuracy + "%")
//    println("Class 1 Accuracy:" + class_1_predict + "/" + class_1_sum + " " + class_1_accuracy + "%")
//    println("Average Accuracy:" + average_accuracy + "%")
//    println("Accuracy:" + sum_accuracy + "%")
//    import spark.implicits._
//    val df = Seq(
//      ("class_0_accuracy", class_0_accuracy.toString),
//      ("class_1_accuracy", class_1_accuracy.toString),
//      ("average_accuracy", average_accuracy.toString),
//      ("sum_accuracy", sum_accuracy.toString)
//    ).toDF("name","accuracy")
//    df.coalesce(1).write.mode(SaveMode.Overwrite).option("sep","#").option("header","true").csv(p.output_pt+"_text")
//  }
}
